...
//
// Two agents (engagement, renewal) draft content a human approves. Two
// (workshopReminder, renewalCycleOpener) act fully autonomously — see each
// agent's own file for why those specific two were judged safe to automate.
@Injectable()
export class AgentRunnerService {
  ...
  async runAll(): Promise<{ engagement: number; renewal: number; workshopReminder: number; renewalCycleOpener: number }> {
    ...
    return { engagement, renewal, workshopReminder, renewalCycleOpener };
  }
}
